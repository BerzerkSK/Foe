...
class CechPresenter extends BasePresenter{
    ...
    public function createComponentEditCechForm() {
        ...
        $form->addText('uroven', 'Úroveň cechu: ')
                ->setAttribute('class', 'form-control')
		->setRequired(TRUE)
                ->addRule(Form::INTEGER, 'Vložte číselnú hodnotu.');
	...
	$tyzden = strftime('%W', Time());
        ...
        $data = "";
		$datum = "12/30/".$termin_rok;

        $pocet_tyzdnov = strftime("%V", strtotime($datum));

		for($i = 1; $i <= $pocet_tyzdnov; $i++) {
            $data[$i] = $i;
			if($i<10) {
				$data[$i] = "0".$data[$i];
			}
    ...
    public function baseLawFormSucceeded(Nette\Application\UI\Form $form, $values) {
        if($values->termin_rok <> $values->old_rok) {
			if($values->termin_rok <> strftime('%Y', Time())) {
				$values->termin_tyzden = "01";
			}
			else {
				$values->termin_tyzden = strftime('%W', Time());
				if(strlen($values->termin_tyzden) == 1) {
					$values->termin_tyzden = "0".$values->termin_tyzden;
				}
			}
		}
