...
// vyber a priprav data do selectov
        ...
        $user['role'] = $this->role;
        $user['cech'] = $this->cech; // simulacia cechu prihlaseneho hraca
        ...
        $form->addText('relikvie', 'Chrám relikvií:', 1, 2)
                ->setRequired(TRUE)
                ->addRule($form::RANGE, 'Hodnota musí byť v rozsahu od %d do %d.',[0,$this->max_uroven_VB]); // ak zmenim max lvl, treba zmenit hodnoty v CechPresenter
        
        $form->addText('alcatraz', 'Alcatraz:', 1, 2)
                ->setRequired(TRUE)
                ->addRule($form::RANGE, 'Hodnota musí byť v rozsahu od %d do %d.',[0,$this->max_uroven_VB]); // ak zmenim max lvl, treba zmenit hodnoty v CechPresenter

        $form->setDefaults(['role' => 'clen', 'era' => 1, 'hvezdaren' => 0,
                            'atomium' => 0, 'obluk' => 0, 'relikvie' => 0, 'alcatraz' => 0]);
...
//        $form->setMethod('GET');
	...
	if($user['role'] !== 'clen') {
        ...
        $form->setDefaults(['role' => $user_data['role'],
                                ...
                                'relikvie' => $user_data['relikvie'],
                                'alcatraz' => $user_data['alcatraz'],
                                'nick' => $user_data['nick'],
    ...
    public function actionEdit($user_id = NULL) {
        ...
        if($this->logged) {
            ...
            if($this->is_role_admin && $user_edited['role'] == 'superadmin') {
                $this->flashMessage('S aktuálnymi oprávneniami nemôžeš zvoleného užívateľa upravovať.');
                $this->redirect('Cech:showAll');
            }
            if($this->is_role_admin && $this->cech <> $user_edited['cech']) {
                $this->flashMessage('S aktuálnymi oprávneniami nemôžeš zvoleného užívateľa upravovať.');
                $this->redirect('Cech:showAll');
            }
            if($this->is_role_clen && $user_id <> $this->user_id){
