...
class Container_23a8d5114d extends Nette\DI\Container
{
	protected $meta = [
		'types' => [
	...
	{
		$service = new App\Presenters\UserPresenter($this->getService('database.default.connection'));
		$service->injectPrimary($this, $this->getService('application.presenterFactory'),
			$this->getService('routing.router'), $this->getService('http.request'),
			$this->getService('http.response'), $this->getService('session.session'),
			$this->getService('security.user'), $this->getService('latte.templateFactory'));
	...
	{
		$service = new Nette\Database\Connection('mysql:host=127.0.0.1;dbname=foe', 'root',
			NULL, ['lazy' => TRUE]);
	...
	{
		return new Container_23a8d5114d_Nette_Bridges_ApplicationLatte_ILatteFactoryImpl_latte_latteFactory($this);
	}
...
class Container_23a8d5114d_Nette_Bridges_ApplicationLatte_ILatteFactoryImpl_latte_latteFactory implements Nette\Bridges\ApplicationLatte\ILatteFactory
{
	private $container;


	public function __construct(Container_23a8d5114d $container)
	{
